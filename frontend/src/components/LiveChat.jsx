...
import { Send, Trophy, Star, ShieldCheck, Heart, Gavel } from "lucide-react";
...
const USER_COLORS = [
  ...
  "text-emerald-500", "text-indigo-500", "text-cyan-500"
];
...
export const LiveChat = ({ auctionId }) => {
  ...
  return (
    <div className="flex flex-col h-[600px] border border-ink/10 bg-white shadow-2xl overflow-hidden font-sans">
      {/* Header */}
      <div className="p-4 border-b border-ink/5 bg-white flex items-center justify-between">
        ...
        <AnimatePresence initial={false}>
          {messages.map((msg) => (
            <motion.div 
              ...
              className={`flex gap-3 items-start p-2 transition-colors ${
                msg.isPayment ? 'bg-gold/5 border-l-2 border-gold -mx-2 px-4' : ''
              }`}
            >
              <div className={`w-8 h-8 rounded-full flex-shrink-0 flex items-center justify-center text-[10px] font-bold bg-white border border-ink/5 shadow-sm ${
              ...
              <div className="flex flex-col">
                <div className="flex items-baseline gap-2">
                  <span className={`text-[11px] font-bold ${
                    msg.isSystem ? 'text-gold uppercase tracking-tighter' : getUserColor(msg.senderId)
                  }`}>
                    {msg.senderName}
                  </span>
                ...
                <p className={`text-sm leading-relaxed break-words ${
                  msg.isPayment ? 'font-bold text-ink' : 'text-ink/80'
                }`}>
                  {msg.text}
                </p>
              </div>
          ...
          <form onSubmit={sendMessage} className="flex flex-col gap-3">
            <div className="relative">
              <input
                ...
                className="w-full pl-0 pr-10 py-2 text-sm border-b border-ink/10 outline-none focus:border-gold transition-all bg-transparent"
                maxLength={200}
              />
              <button 
                type="submit"
                disabled={!inputText.trim()}
                className="absolute right-0 top-1/2 -translate-y-1/2 p-2 text-ink/40 hover:text-gold disabled:opacity-30 transition"
              >
                <Send className="w-4 h-4" />
              </button>
            </div>
            <div className="flex justify-between items-center">
               <span className="text-[9px] text-ink/20 uppercase tracking-widest font-bold">
