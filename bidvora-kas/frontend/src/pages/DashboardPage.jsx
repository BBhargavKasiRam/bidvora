  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        let listedCount = 0;
        if (user?.role === "seller") {
          const myListingsRes = await api.get(`/auctions?seller_id=${user.id}`);
          listedCount = myListingsRes.length;
        }

        setRecentAuctions(auctionsRes.slice(0, 3));
        setStats({
          active: auctionsRes.length,
          won: ordersRes.length,
          listed: listedCount
        });
      } catch (err) {
        console.error(err);
