...
import { Package, Truck, CheckCircle, ExternalLink, Trophy, Clock, CreditCard } from "lucide-react";
...
export const OrdersPage = () => {
  ...
  const handleCheckout = async (auctionId, price) => {
    try {
      setError("");
      setMessage("Processing payment...");
      await api.post("/orders/checkout", { auction_id: auctionId, final_price: price });
      setMessage("Payment successful!");
      fetchOrders(); // Refresh orders to get updated transaction info
    } catch (err) {
              ...
              <div className="grow flex flex-col justify-between py-2">
                ...
                <div className="flex gap-6 mt-10">
                  {order.transaction_id ? (
                    ...
                    <button
                      onClick={() => handleCheckout(order.id, order.price)}
                      className="flex items-center gap-3 px-8 py-3 bg-gold text-ink text-[10px] uppercase tracking-widest font-bold hover:bg-ink hover:text-paper transition-colors"
                    >
