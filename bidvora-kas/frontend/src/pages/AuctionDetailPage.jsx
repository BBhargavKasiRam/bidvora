  ...
  useEffect(() => {
    ...
    const checkWatchlist = async () => {
      ...
      try {
        ...
        setIsWatchlisted(watchlist.some(item => item.auction_id === Number(id)));
      } catch (err) {
        console.error(err);
      }
    };
    checkWatchlist();
  }, [fetchAuction, user, id]);
  ...
  const toggleWatchlist = async () => {
    ...
    try {
      ...
      if (isWatchlisted) {
        ...
        await api.post('/watchlist', { auction_id: Number(id) });
              ...
              <button
                ...
                className={`p-4 border rounded-full transition-colors ${
                  isWatchlisted 
                    ...
                    : "bg-white border-ink/10 text-ink/40 hover:text-red-500 hover:border-red-200"
                }`}
                title={isWatchlisted ? "Remove from Watchlist" : "Add to Watchlist"}
              >
                <Heart className="w-6 h-6" fill={isWatchlisted ? "currentColor" : "none"} />
              </button>
            )}
          </div>
