...
// ─── BACKGROUND TASKS ───────────────────────────────────────────────────────
// Periodically close auctions whose end time has passed
setInterval(() => {
  const sql = "UPDATE auctions SET status = 'ended' WHERE end_time <= NOW() AND status = 'active'";
  db.query(sql, (err, result) => {
    if (err) {
      console.error("Error auto-closing auctions:", err);
    } else if (result.affectedRows > 0) {
      console.log(`Auto-closed ${result.affectedRows} ended auction(s).`);
    }
  });
